...
for sample in `cat runList.tsv`; do
    ...
    for i in "${genes[@]}"; do
        p=`echo "$p+1" | bc` 
        bwa mem -t 72 -k 10 -L 0 -B 1 -O 1 -T 18 /refgenome/ref.fa $sample/$sample.$p.R1.fastq $sample/$sample.$p.R2.fastq 2> $sample/log/bwa.TR$i.log | samtools view -Sb -  2>> $sample/log/bwa.TR$i.log | samtools sort - -o $sample/TR$i.srt.bam 2>> $sample/log/bwa.TR$i.log
        samtools index $sample/TR$i.srt.bam
        umi_tools group -I $sample/TR$i.srt.bam --edit-distance-threshold=2 --group-out=$sample/TR$i.grouped.tsv --log=$sample/log/umi_tools.TR$i.log --method=adjacency
