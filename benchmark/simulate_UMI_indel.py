...
def sim_UMI(simf, umi_len, acc, pN, oN, do_indel):
    ...
    for umi_length in umi_lengthses:
        ...
        for trip in tripses:
            for off_accuracy in off_accuracieses:
                ...
                num_sig = 0
                for i in range(par_N):
                    rn_off = round(np.random.negative_binomial(n,p)*p)
                    if rn_off == 0:
                        num_sig +=1
...
sim_UMI(simf, umi_len, acc, pN, oN, do_indel)
